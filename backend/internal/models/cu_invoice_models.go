...
// InvoiceCreateRequest 客户申请发票请求结构
type InvoiceCreateRequest struct {
	...
	Title         string  `json:"title" binding:"required,max=200"`                                      // 发票抬头，必填，最大200字符
	TaxpayerID    *string `json:"taxpayer_id,omitempty"`                                                 // 纳税人识别号，企业发票必填，个人发票可为空
	Content       string  `json:"content" binding:"required,max=200"`                                    // 开票内容，必填，最大200字符
...
type InvoiceListRequest struct {
	...
	Search     string `form:"search" binding:"omitempty"`                  // 搜索关键词，支持发票号或订单号模糊匹配
	ApplyDate  string `form:"apply_date" binding:"omitempty"`              // 申请日期筛选，格式YYYY-MM-DD
